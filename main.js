...
const LARAVEL_API_BASE = 'https://626e82e7cf76.ngrok-free.app/api';
...
    return;
  }
// ... (sendMessage function content remains the same)
  const payload = {
...
// ... (fetchFromLaravel function content remains the same)
...
    const response = await fetch(`${LARAVEL_API_BASE}/babies`, { // Targetting the POST /api/babies route
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
            // Check if the user wants to list all babies (GET request)
...
          // --- Logic for saving a NEW baby (POST request) ---
          
          // Using a static structure and the user's input as the first_name
          const babyData = {
            first_name: incomingText,
            guardian_id: 1, // Assumes guardian with ID 1 exists
            gender: "Male",
            immunization_status: "Pending", 
            last_vaccine_received: "BCG",
            next_appointment_date: "2025-12-30 10:00:00", // Laravel-friendly format
            date_of_birth: "2024-10-15", // Laravel-friendly format
            nationality: "Kenya"
          };

          await saveBabyToLaravel(babyData);
          await sendMessage(senderId, `✅ Baby "${incomingText}" has been registered successfully.`);
